...
public class NewKCPQuery
{
    private static final Logger log = LogManager.getLogger(NewKCPQuery.class);
            ...
            throws Exception
    {
        ...
        final SparkContext sparkContext = spatialRDD.spatialPartitionedRDD.context();
        ...
        final JavaRDD<PriorityQueue<DistanceAndPair<U, T>>> joinResult;

        verifyCRSMatch(queryRDD, spatialRDD);

        queryRDD.rawSpatialRDD.cache();

        Double bound = calculateBound(queryRDD, spatialRDD, joinParams.k, joinParams.algorithm, joinParams.sample);

        System.out.println("Bound "+bound);

        CircleRDD circleRDD = new CircleRDD(queryRDD, bound);
    ...
    private static <U extends Geometry, T extends Geometry> Double calculateBound(SpatialRDD<U> queryRDD, SpatialRDD<T> spatialRDD, int numberK, KCPQAlgorithm algorithm, Double sample) {
